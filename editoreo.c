...
#define EDITOREO_VERSION "0.0.1"
#define EDITOREO_TAB_STOP 8
#define EDITORIO_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; j++) {
        if(row->chars[j] == '\t') 
            rx += (EDITOREO_TAB_STOP - 1) - (rx % EDITOREO_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for(cx = 0; cx < row->size; cx++) {
        if(row->chars[cx] == '\t')
            cur_rx += (EDITOREO_TAB_STOP - 1) - (cur_rx % EDITOREO_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(EDITOREO_TAB_STOP - 1) + 1);

    int idx = 0;
    for(j = 0; j < row->size; j++) {
        if(row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while(idx % EDITOREO_TAB_STOP != 0) row->render[idx++] = ' ';
        } else {
    ...
    for(y = 0; y < E.screenrows; y++) {
        ...
        if(filerow >= E.numrows) {
            if(E.numrows == 0 && y == E.screenrows / 3) {
                ...
                "EDITORIO -- VERSION %s", EDITOREO_VERSION);
...
void editorProcessKeyPress() {
    static int quit_times = EDITORIO_QUIT_TIMES;
    ...
    quit_times = EDITORIO_QUIT_TIMES;   
}
